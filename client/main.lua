...
local function openMenu(allowedMenus)
    ...
    PlayerData = RXCore.Functions.GetPlayerData()
...
local function SaveSkin()
    ...
    TriggerServerEvent("rx-clothing:saveSkin", model, clothing)
end
...
local function reloadSkin(health)
    ...
    local gender = RXCore.Functions.GetPlayerData().charinfo.gender
    ...
    TriggerServerEvent("rx-clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("rx-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
...
local function getOutfits(gradeLevel, data)
    local gender = "male"
    if RXCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    RXCore.Functions.TriggerCallback('rx-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('RXCore:Client:UpdateObject', function()
	RXCore = exports['rx-core']:GetCoreObject()
end)

RegisterNetEvent('rx-clothing:client:openMenu', function()
...
RegisterNetEvent('rx-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
        action = "reloadMyOutfits",
        outfits = myOutfits
    })
end)
RegisterNetEvent('rx-clothes:client:CreateFirstCharacter', function()
    RXCore.Functions.GetPlayerData(function(pData)
...
RegisterNetEvent("rx-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('rx-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('rx-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('rx-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if RXCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        RXCore.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
RegisterNetEvent("rx-clothing:client:adjustfacewear", function(type)
    if RXCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
...
RegisterNetEvent('RXCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("rx-clothes:loadPlayerSkin")
    PlayerData = RXCore.Functions.GetPlayerData()
    loadStores()
--    RXCore.Shared.Jobs = exports['rx-jobs']:AddJobs()
end)
RegisterNetEvent('RXCore:Client:OnJobUpdate', function(JobInfo)
    PlayerData.job = JobInfo
end)
RegisterNetEvent('RXCore:Client:OnGangUpdate', function(GangInfo)
    PlayerData.gang = GangInfo
end)
RegisterNetEvent('rx-clothing:client:openOutfitMenu', function()
    RXCore.Functions.TriggerCallback('rx-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('rx-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNUICallback('TrackerError', function(_, cb)
    RXCore.Functions.Notify(Lang:t("notify.error_bracelet"), "error")
    cb('ok')
end)
RegisterNUICallback('saveOutfit', function(data, cb)
    local ped = PlayerPedId()
    local model = GetEntityModel(ped)
    TriggerServerEvent('rx-clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('rx-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('rx-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    RXCore.Functions.Notify(Lang:t('notify.info_deleteOutfit', {outfit = data.outfitName}))
    cb('ok')
end)
RegisterNUICallback('setCurrentPed', function(data, cb)
    local playerData = RXCore.Functions.GetPlayerData()
...
Citizen.CreateThread(function()
    for k, _ in pairs (Config.Stores) do
        local coords = Config.Stores[k].coords
        local blip = AddBlipForCoord(coords.x, coords.y, coords.z)
        local sprite, color, label

        if Config.Stores[k].shopType == "clothing" then
            sprite, color, label = 366, 47, Lang:t('store.clothing')
        elseif Config.Stores[k].shopType == "barber" then
            sprite, color, label = 71, 0, Lang:t('store.barber')
        elseif Config.Stores[k].shopType == "surgeon" then
            sprite, color, label = 71, 0, Lang:t('store.surgeon')
        end
    ...
    if Config.UseTarget then
        CreateThread(function()
            for k, v in pairs(Config.Stores) do
                ...
                exports['rx-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                    name = v.shopType .. k,
                    debugPoly = false,
                    minZ = v.coords.z-1,
                    maxZ = v.coords.z+1,
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                elseif not RXCore.Shared.QBJobsStatus then
                    action = function()
                        customCamLocation = v.cameraLocation
                ...
                exports['rx-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
	    ...
	    for k, v in pairs(Config.OutfitChangers) do
                exports['rx-target']:AddBoxZone('OutfitChangers_'..k, v.coords, v.length, v.width, {
                    ...
                    options = {
                        {
                            type = "client",
                            event = "rx-clothing:client:openOutfitMenu",
                            icon = "fas fa-sign-in-alt",
            ...
            clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
                if isPointInside then
                    ...
                    if zoneName == 'surgeon' then
                        exports['rx-core']:DrawText('[E] - '..Lang:t("store.surgeon"), 'left')
                    elseif zoneName == 'clothing' then
                        exports['rx-core']:DrawText('[E] - '..Lang:t("store.clothing"), 'left')
                    elseif zoneName == 'barber' then
                        exports['rx-core']:DrawText('[E] - '..Lang:t("store.barber"), 'left')
                    elseif zoneName == 'outfit' then
                        exports['rx-core']:DrawText('[E] - '..Lang:t("store.outfitchanger"), 'left')
                    end
                else
                    inZone = false
                    ...
                    if PlayerData.gang and PlayerData.gang.name or (not RXCore.Shared.QBJobsStatus and PlayerData.job.name) then
                        ...
                        clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
                            if isPointInside then
                                local zoneID = tonumber(RXCore.Shared.SplitStr(zone.name, "_")[2])
                                local job = Config.ClothingRooms[zoneID].isGang and PlayerData.gang.name or (not RXCore.Shared.QBJobsStatus and PlayerData.job.name)
                ...
                if inZone then
                    ...
                    elseif not RXCore.Shared.QBJobsStatus then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local gradeLevel = clothingRoom.isGang and PlayerData.gang.grade.level or (not RXCore.Shared.QBJobsStatus and PlayerData.job.grade.level)
